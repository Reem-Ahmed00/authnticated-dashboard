...
export default function DashboardOverview() {
  ...
  const posts = useSelector((state) =>
    state.posts.posts.filter((post) => post.username === user.username)
  );
  ...
  return (
    <div className="container py-5">
      ...
      <div className="card shadow mb-4 p-4 d-flex flex-column align-items-center">
        <img
          src={user?.profileImage || "/images/user.png"} // Use default image if profileImage is missing
          alt="Profile"
          ...
          {posts.map((post) => (
            <div key={post.id} className="col-md-6">
              <div className="card shadow-sm">
                <img
                  src={post.image || "https://via.placeholder.com/300x200"} // Use placeholder if no image
                  alt={post.title}
